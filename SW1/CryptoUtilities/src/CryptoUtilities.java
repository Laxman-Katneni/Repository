...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
         ...
         */
        if (m.isZero()) // base Test
        {
            m.clear();
        } else {

            NaturalNumber mCpy = m.newInstance();
            mCpy.copyFrom(m);

            powerMod(n, new NaturalNumber2(1), mCpy);
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        NaturalNumber one = p.newInstance();
        NaturalNumber two = p.newInstance();
        NaturalNumber nCpy = p.newInstance();
        NaturalNumber pCpy = p.newInstance();
        ...
        if (p.isZero()) // Base test
        {
            ...
            if (isEven(n)) {
                ...
                powerMod(nCpy, pCpy, m); // freeLunch.powerMod(nCopy, pCopy, m);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        powerMod(wCpy, two, nCpy);

        if (wCpy.canConvertToInt() && wCpy.toInt() == 1) // Test # 1
        {
            return true;
        } else {
            ...
            return w.canConvertToInt() && w.toInt() != 1; //Test #3
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        for (int i = 0; i < 50; i++) // 50 -> Num of candidates
        {
            ...
            w = randomNumber(n);
            while (!(w.compareTo(new NaturalNumber2(1)) > 0)
                    || !(w.compareTo(nCpy) < 0)) {
                w.transferFrom(randomNumber(n));
            }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        while (result) {
            ...
            System.out.println("The value of n is " + n); // simplewriter is not availble in this domain

            n.add(two);

            if (isPrime1(n)) // we found a prime and we need to breakthrough the loop
            {
                result = false;
            }
        }
