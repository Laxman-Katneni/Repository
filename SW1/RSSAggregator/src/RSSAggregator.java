...
public final class RSSAggregator {
    ...
    private static void indexOutput(XMLTree feeds, SimpleWriter out) {
        ...
        for (int i = 0; i < feeds.numberOfChildren(); i++) {
            ...
            if (feeds.child(i).isTag()) {
                if (feeds.child(i).label().equals("feed")) {
                    if (feeds.child(i).hasAttribute("url")
                            && feeds.child(i).hasAttribute("file")) {

                        SimpleWriter fileName = new SimpleWriter1L();

                        String name = "No name";

                        if (feeds.child(i).hasAttribute("name")) {
                            name = feeds.child(i).attributeValue("name");
                        }

                        fileName = processFeed(
                                feeds.child(i).attributeValue("url"),
                                feeds.child(i).attributeValue("file"), out);
                        String htmlName = fileName.name();
                        out.println("<li><a href=\"" + htmlName + "\">" + name
                                + "</a></li>");
                        fileName.close();
                    }
     ...
     * </pre>
     */
    private static SimpleWriter processFeed(String url, String file,
            SimpleWriter out) {
    ...
    private static void processItem(XMLTree item, SimpleWriter out) {
        ...
        if (pubDateNum != -1) {
            ...
            pubDate = item.child(pubDateNum).child(0).label().substring(0, 14);
        }
        ...
        if (linkNum != -1) {
            if (sourceNum >= 0
                    && item.child(sourceNum).numberOfChildren() > 0) {
    ...
    public static void main(String[] args) {
        ...
        out.println("Enter XML file with feeds");
